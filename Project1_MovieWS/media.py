...
Udacity FSWD Course 1 
Mini project 1 - Movie Webpage
@author: ALLAN
"""
...
def get_data(movie):
    ...
    data = urlopen("http://www.omdbapi.com/?t=" + movie + "&apikey=883d566b").read()
    raw_data = str(data, 'utf-8-sig')
    json_data = json.loads(raw_data)
    return(json_data)

def apply_movie(movie_title, trailers):
    ### given movie titel and trailer list, organizes the data and applies the Movie class.
    mov_data = get_data(movie_title)
    movie = Movie(mov_data["Title"], mov_data["Plot"], mov_data["imdbRating"], mov_data["Runtime"], 
